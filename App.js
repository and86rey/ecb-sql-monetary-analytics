...
const SCENARIOS = {
    yoy: {
        ...
        insight: "<strong>Case: Inflationary Risk KPI.</strong><br>Calculated using <b>SQL Window Functions</b>. Spotting liquidity bubbles is key for Investment Strategy.",
        label: "YoY Growth %"
    },
    raw: {
        sql: "SELECT period, value FROM obs ORDER BY period DESC LIMIT 24;",
        insight: "<strong>Case: Market Liquidity.</strong><br>Direct extraction of M1 levels. Represents 'dry powder' in the Eurozone economy.",
    ...
    ma: {
        ...
        FROM obs ORDER BY period DESC;`,
        insight: "<strong>Case: Trend Smoothing.</strong><br>Using <b>Moving Averages</b> to identify long-term strategic trends over seasonal noise.",
...
async function init() {
    ...
    try {
        // 1. Setup DuckDB Engine first (it's fast if done correctly)
        ...
        // 2. Fetch Data as TEXT (Fastest)
        updateStatus("Fetching ECB Live Data...", "info");
        const resp = await fetch(`https://data-api.ecb.europa.eu`);
        const csvText = await resp.text();

        // 3. Register File and Ingest via DuckDB Native CSV Loader (Blazing Fast)
        ...
        updateStatus("❌ Error: " + e.message, "bad");
        console.error(e);
    }
}
...
function renderTable(data) {
    ...
    <tbody>${data.slice(0, 8).map(row => `<tr>${cols.map(c => {
        let val = row[c];
...
function renderChart(data, label) {
    ...
    chart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: d.map(i => i[cols] instanceof Date ? i[cols].toISOString().split('T')[0] : i[cols]),
